...
class Util:
    ...
    def generate_confusion_matrix(self, predictions, test_df):
        ...
        TPR = (
            statistics["TP"] / (statistics["TP"] + statistics["FN"])
            if (statistics["TP"] + statistics["FN"]) > 0
            else 0
        )
        TNR = (
            statistics["TN"] / (statistics["TN"] + statistics["FP"])
            if (statistics["TN"] + statistics["FP"]) > 0
            else 0
        )

        # Calculate Accuracy
        accuracy = (
            (statistics["TP"] + statistics["TN"])
            / (
                statistics["TP"]
                + statistics["TN"]
                + statistics["FP"]
    ...
    def resample_data_with_smote(self, df, training_features):
        ...
        feature_resampled, label_resampled = smote.fit_resample(
            feature_data, label_data
        )

        # Combine the resampled features and target into a DataFrame
        df_resampled = pd.DataFrame(
            feature_resampled, columns=feature_resampled.columns
        )
