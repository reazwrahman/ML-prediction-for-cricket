...
class BowlingDataGenerator: 
    ...
    def generate_raw_dfs(self):  
        ...
        for i in range (len(self.filenames)):
            ...
            bowling_columns = ['Innings Player','Opposition', 'Ground', 'Innings Date', 'Country',
                    'Innings Overs Bowled', 'Innings Bowled Flag', 'Innings Wickets Taken','Innings Wickets Taken Buckets',
                   'Innings Economy Rate']
            
            df = df[bowling_columns]  
            new_bowling_column = ['player', 'opposition', 'ground', 'date', 'country',
                'overs', 'bowled_flag','wickets', 'bucket', 'economy_rate'] 
    ...
    def do_basic_cleanup(self):
        ...
        self.df['overs'] = self.df['overs'].apply(self.__convert_overs) ## convert overs to decimal system
    ...
    def calculate_wickets_per_game(self): 
        ...
        innings_count=pd.NamedAgg(column='bowled_flag', aggfunc=lambda x: (x == 1).sum())
        ).reset_index()
    
        grouped['career_wickets_per_game'] = round(grouped['total_wickets'] / grouped['innings_count'],2) 
        
        self.df = pd.merge(self.df, grouped[['player', 'career_wickets_per_game']], on='player')  
        self.df['career_wickets_per_game'] = pd.to_numeric(self.df['career_wickets_per_game'], errors='coerce') 
    ...
    def calculate_recent_wickets_per_game(self): 
        ...
        grouped['recent_wickets_per_game'] = round(grouped['total_wickets'] / grouped['innings_count'],2)

        # Merge the average wickets per game back into the original DataFrame
        self.df = pd.merge(self.df, grouped[['player', 'recent_wickets_per_game']], on='player', how='left')  
    ...
    def calculate_career_strike_rate(self): 
        ...
        grouped['career_strike_rate'] = round(grouped['balls_bowled'] / grouped['total_wickets'],2) 
        
        self.df = pd.merge(self.df, grouped[['player', 'career_strike_rate']], on='player')  
        self.df['career_strike_rate'] = pd.to_numeric(self.df['career_strike_rate'], errors='coerce')  
        
        # Replace inf values  
        max_sr = self.df['career_strike_rate'].replace([np.inf, -np.inf], np.nan).max()
        self.df['career_strike_rate'] = self.df['career_strike_rate'].replace(np.inf, max_sr) 
    ...
    def calculate_recent_strike_rate(self): 
        ...
        grouped['recent_strike_rate'] = round(grouped['balls_bowled'] / grouped['total_wickets'],2) 
        
        
        # Merge the average wickets per game back into the original DataFrame
        self.df = pd.merge(self.df, grouped[['player', 'recent_strike_rate']], on='player', how='left')   
        
        # Replace inf values  
        max_sr = self.df['career_strike_rate'].replace([np.inf, -np.inf], np.nan).max()
        self.df['recent_strike_rate'] = self.df['recent_strike_rate'].replace(np.inf, max_sr) 
...
class BowlingDataUtil: 
    def __init__(self): 
        ...
        self.training_data_generator = BowlingDataGenerator(filenames=self.training_files)
        ...
        self.selected_features=['opposition', 'ground', 'country', 'career_wickets_per_game',
                                 'career_wickets_per_game', 'career_strike_rate', 
                                 'recent_strike_rate']  
